...
class SmMsUploader(ImageUploader):
    ...
    def upload(self, file: bytes, filename: str) -> UploadResponse:
        try:
            ...
            return UploadResponse(success=True, code="success", message="Upload success", data=image_metadata)
...
class PicGoUploader(ImageUploader):
    ...
    def upload(self, file: bytes, filename: str) -> UploadResponse:
        ...
        try:
            ...
            # 处理网络请求相关错误
            raise UploadError(
                message=f"Upload request failed: {str(e)}", error_type=UploadErrorType.NETWORK_ERROR, original_error=e
            )
        except (KeyError, ValueError, TypeError) as e:
            # 处理响应解析错误
            raise UploadError(
                message=f"Invalid response format: {str(e)}", error_type=UploadErrorType.PARSE_ERROR, original_error=e
            )
        except UploadError:
            ...
            # 处理其他未预期的错误
            raise UploadError(message=f"Upload failed: {str(e)}", error_type=UploadErrorType.UNKNOWN, original_error=e)
    ...
    def upload(self, file: bytes, filename: str) -> UploadResponse:
        ...
        try:
            ...
            if not result or not isinstance(result, list) or len(result) == 0:
                raise UploadError(message="Invalid response format", error_type=UploadErrorType.PARSE_ERROR)

            # 获取文件URL
            file_path = result[0].get("src")
            if not file_path:
                raise UploadError(message="Missing file URL in response", error_type=UploadErrorType.PARSE_ERROR)
...
class ImageUploaderFactory:
    ...
    def create(provider: str, **credentials) -> ImageUploader:
        if provider == "smms":
            ...
            return CloudFlareImgBedUploader(credentials["auth_code"], credentials["base_url"])
