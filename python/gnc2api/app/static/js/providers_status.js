...
function copyProviders(type) {
    const keys = Array.from(document.querySelectorAll(`#${type}Providers .key-text`)).map(span => span.textContent.trim());
    const jsonKeys = JSON.stringify(keys);

    copyToClipboard(jsonKeys)
        .then(() => {
            showCopyStatus(`已成功复制${type === 'valid' ? '有效' : '无效'}接口到剪贴板`);
        })
        .catch((err) => {
...
async function verifyProvider(provider, button) {
    try {
        ...
        const service = btoa(provider)
        const response = await fetch(`/gemini/v1beta/verify/${service}`, {
            method: 'POST'
        });
