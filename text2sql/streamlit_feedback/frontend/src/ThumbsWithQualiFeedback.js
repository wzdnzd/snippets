...
export function ThumbsWithQualiFeedback(props) {
    ...
    const verify = (score, text) => {
        return score !== "👎" || (text !== undefined && text !== null && text?.trim() !== "")
    }
    ...
    const submitTextLabel = props.submitTextLabel?.trim() || "Submit";
    const cancelTextLabel = props.cancelTextLabel?.trim() || "Cancel";
        ...
        return (
            <Box paddingY={0.5} height={140} component="form" sx={{ "& .MuiTextField-root": { m: 1, width: "50ch" } }} noValidate autoComplete="off">
                <Stack direction="row" spacing={1} justifyContent={props.align}>
                    ...
                    {submitted === false && thumbScore !== null ? <Button sx={{ color: thumbScore === "👍" ? colors["colorUp"] : colors["colorDown"] }} variant="outlined" size="small" onClick={handleSubmission} disabled={!verify(thumbScore, inputText)}>{submitTextLabel}</Button> : null}
                    {submitted === false && thumbScore !== null ? <Button sx={{ color: colors["colorGrey"] }} variant="outlined" size="small" onClick={handelCancel}>{cancelTextLabel}</Button> : null}
                </Stack>
            </Box>
        )
    }
