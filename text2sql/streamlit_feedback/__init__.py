...
def streamlit_feedback(
    ...
    sumbit_text_label: str or None
        An optional label to add to the submit button. If None, the default label will be "Submit".
    cancel_text_label: str or None
        An optional label to add to the cancel button. If None, the default label will be "Cancel".
    ...
    if feedback_type == "thumbs":
        ...
        raise NotImplementedError(
            f"feedback_type='{feedback_type}' not implemented. Please select either" " 'thumbs' or 'faces'."
        )
    if align not in ["flex-end", "center", "flex-start"]:
        raise NotImplementedError(
            f"align='{align}' not implemented. Please select either 'flex-end'," " 'center' or 'flex-start'."
        )
    if max_text_length and optional_text_label is None:
        raise NotImplementedError("max_text_length requires optional_text_label to be set.")

    if key is None:
        key = feedback_type

    if not submit_text_label or not isinstance(submit_text_label, str):
        submit_text_label = "Submit"

    if not cancel_text_label or not isinstance(cancel_text_label, str):
        cancel_text_label = "Cancel"
    ...
    component_value = _component_func(
        ...
        cancel_text_label=cancel_text_label,
    )
